...
#        • Thresholds placed 10% beyond the platform-matched healthy-control
#          z-score extremes
...
#   6. Read and normalize the configured metadata file a second time for legacy
#      compatibility (`meta_df` is not joined into the active output).
#   7. Write the final table and supporting audit/statistics files.
#
# Inputs:
#   • Fragmentomics_data/Dilution_series/Metadata_dilution_series.csv
#   • Historical dilution nucleosome, insert-size, and fragment-score TSVs
#     under Fragmentomics_data/Dilution_series/
#   • Historical healthy-control TSVs under Fragmentomics_data/Normals/
#   • Spring 2026 XPlus dilution and matched-control TSVs resolved by helpers.R
...
#         Results_Fragmentomics/Dilution_series/key_fragmentomics_info_dilution_series.rds
#         Results_Fragmentomics/Dilution_series/fragmentomics_platform_reference_thresholds_dilution.csv
#         Results_Fragmentomics/Dilution_series/fragmentomics_threshold_direction_sensitivity_audit.csv
#         Results_Fragmentomics/Dilution_series/fragmentomics_platform_harmonization_dilution_audit.csv
#         Results_Fragmentomics/Dilution_series/Griffin/griffin_per_site_metrics_dilution.tsv
#         Results_Fragmentomics/Dilution_series/Griffin/griffin_per_site_stats_dilution.tsv
#         Results_Fragmentomics/Dilution_series/spring2026_unmatched_dilution_fragmentomics_samples.csv
#           (only when unmatched samples exist)
#
# How to run:
...
#   The t-tests compare sample rows and are not used as replicate-aware LOD
#   estimates. BH-adjusted values are calculated in the in-memory `fdr.data`
#   object but are not joined to either exported table. Active Threshold.* calls
#   use the ratio-appropriate split at fold-change 1; the historical >= 0 / > 0
#   rule is retained only in a separate sensitivity audit.
#
...
for (site in all.sites) {
  ...
  # background; > 1.0 indicates elevated coverage at the site centre. This is a
  # difference shifted by 1, not a ratio or a universal accessibility direction.
...
# Audit the corrected active direction rule against the historical rule. The
# legacy columns below reproduce fold-change > 0 for comparison only; they do
# not feed the dilution models or manuscript figures.
