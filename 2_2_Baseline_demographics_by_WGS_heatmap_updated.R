...
#
# Purpose:
#   Generate the baseline integrated genomic alteration heatmaps, feature
#   catalogue, and per-variant VAF table used in Extended Data Figure 1 and
#   Supplementary Table 2 sheets A-B:
...
#   the final concordance manuscript panels/tables are generated by script 2_3.
#
#   Heatmap tiles show:
...
#
# Units of analysis:
#   - Heatmap: one eligible patient/baseline display column, containing the BM
#     and/or cfDNA call for each assessed genomic feature.
#   - Supporting concordance tables: one patient-timepoint BM/cfDNA comparison
#     record; empty call sets are treated as exactly concordant.
#   - Feature catalogue: one assessed mutation, CNA, or translocation feature.
#
# Inputs:
...
#   - Cohort assignments loaded by load_final_cohort_assignment() in helpers.R
#   - Jan2025_exported_data/All_feature_data_Sep2025_updated2.rds
#   - Jan2025_exported_data mutation, translocation, and CNA RDS files produced
#     by scripts 1_2 through 1_5
#   - combined_maf_bm_dx.rds and combined_maf_blood_all_muts_updated.rds
#   - Mutation_counts/*_no_RSID.txt and Revision_cfWGS_remove_rsIDs_counts.tsv
#   - Final Tables and Figures/mutation_overlap_support/
#       mutation_overlap_with_cohort.csv (produced by 1_2_Part2)
#   - Final aggregate clinical/FISH table and id_map.rds for annotations
#
# Outputs:
#   - heatmap_output_BM_baseline_updated_15.png (stand-alone BM diagnostic)
#   - heatmap_output_Blood_baseline_updated_15.png (stand-alone cfDNA diagnostic)
#   - Final Tables and Figures/
#       Figure_1B_overlay_heatmap_BM_vs_cfDNA_updated_with_FISH_15.png
#       (Extended Data Figure 1 plotted component)
...
#   - Output_tables_2025_updated/MM_mutations_VAF_by_sample.csv
#       (baseline mutation/VAF rows used in Supplementary Table 2 sheet B)
...
#   stringr, stringdist, readr, grid, ggplot2, tibble, glue, writexl
#
# How to run:
...
#   - Supplementary Table 2 sheet B: baseline BM/cfDNA mutation and VAF rows.
#   - Supplementary Table 2 sheets C-F are generated by script 2_3.
#
# Pipeline role:
...
#   tiles are plasma cfDNA calls. This enables direct inspection of concordance
#   and discordance by patient, assay compartment, alteration class, and cohort.
#
# Author:    Dory Abelman
...
# ATR was interrogated in the WGS callsets but has no qualifying baseline coding
# call for the displayed analysis. Keep it in the assessed catalogue while
# excluding it from the observed-call heatmap rows.
...
# Regression guard for cfDNA-only mutation rows. ATM and RB1 have qualifying
# cfDNA-only baseline calls and must remain visible after cohort/sample filtering.
# The earlier BM-row alignment silently removed these rows even though the calls
# were retained in the per-variant supplementary table.
...
########  Supporting overlap and concordance summaries
# These analyses aid interpretation and QC of the heatmap inputs. Script 2_3 is
# used to generate the manuscript concordance panels and
...
if (!requireNamespace("writexl", quietly = TRUE)) {
  stop("Package 'writexl' is required to export the legacy feature-catalogue XLSX alias.", call. = FALSE)
}
writexl::write_xlsx(
