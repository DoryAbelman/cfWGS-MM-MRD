...
#   ms_copy_artifact() and ms_save_plot() do substantially more than copy or
#   save one file. They update the per-artifact and generated mirrors, refresh
#   indexes and current-final copies, and may render PDF previews. When matching
#   legacy filename variants or byte-identical siblings are found in an output
#   folder, the cleanup helpers remove those copies with unlink() without first
#   creating a backup. CodeOcean_GitHub_Staging mirrors can also be updated when
#   that directory exists. The scientific source_path passed to the helper is
#   not deleted, but organized output/staging copies can be overwritten or
#   removed. Do not call these functions merely to inspect an artifact.
#   ms_write_output_index() is also a writing operation: before rebuilding the
#   indexes it recopies every mapped current-final artifact, then refreshes
#   manifests, reference copies, run snapshots, and figure previews.
#
# Directory layout written by these helpers:
#   01_main_figures/, 02_extended_data_figures/, 03_main_tables/,
#   04_supplementary_tables/
#     Primary per-artifact folders. Every copied file sits under its final
#     figure/table and panel or sheet label.
#
...
#   the directory names and final figure/table labels come from the same
#   artifact map used during validation. Each script still calls the helper
...
ms_find_project_root <- function(start = getwd()) {
  ...
  repeat {
    ...
    is_standalone_repo <-
      file.exists(file.path(current, "run_pipeline.R")) &&
      file.exists(file.path(current, "docs", "manuscript_artifact_source_map.tsv"))
    if (is_standalone_repo) return(current)
...
ms_final_scripts_dir <- function(project_root = ms_find_project_root()) {
  nested <- file.path(project_root, "Scripts_2025", "Final_Scripts")
  if (dir.exists(nested)) nested else project_root
}

ms_output_root <- function(project_root = ms_find_project_root()) {
  # This accessor has side effects: it creates the full output directory tree.
...
ms_artifact_metadata <- function(artifact_id, project_root = ms_find_project_root()) {
  ...
  # The map is expected to contain one row per artifact_id. If duplicates are
  # present, this function uses the first row; validate the map before release.
  hit[1, , drop = FALSE]
}
...
ms_compact_role_label <- function(role, source_path = "") {
  ...
  role_map <- c(
    ...
    test_cohort_mfc_only_figure_panel_png = "test_MFC",
    test_cohort_mfc_only_source_data_csv = "test_MFC_source",
    test_cohort_mfc_only_correlations_csv = "test_MFC_stats",
    test_cohort_mfc_only_counts_csv = "test_MFC_counts",
...
ms_record_manifest <- function(row, project_root = ms_find_project_root()) {
  ...
  if (file.exists(manifest)) {
    # Preserve the history of copy/save events. Repeated runs therefore append
    # repeated artifact rows rather than replacing the previous record.
...
ms_write_output_readme <- function(output_index, script_index, project_root = ms_find_project_root()) {
  ...
  lines <- c(
    ...
    "It mirrors the manuscript-order structure under `reproducible_workflow/outputs`; scientific calculations remain in the numbered scripts.",
    "",
    "## Quick-use folders",
    "",
  ...
  lines <- c(
    ...
    "## Artifact map",
    "",
    "`docs/manuscript_artifact_source_map.tsv` records the original script section, historical filename, final filename, and caveats for each manuscript item."
  )
...
ms_write_output_index <- function(project_root = ms_find_project_root()) {
  source_map <- ms_read_source_map(project_root)
  # Despite its name, this is not an index-only operation. It first overwrites
  # the organized current/frozen/generated copy of every mapped artifact with
  # the currently resolved source file.
