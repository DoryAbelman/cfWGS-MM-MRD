...
#   to the experimental dilution-series libraries and describe feature and
#   model behavior across the tested dilution range. This experiment does not
#   establish a formally validated analytical limit of detection.
#
...
#     4. Plot feature values and model scores against the expected dilution.
#     5. Export source-data CSVs for the dilution-series figures and table.
#
...
#     dilution models.
#   - nested_bm_validation_updated4A.rds and
#     nested_blood_validation_updated4_run2.rds for the historical BM/blood
#     plotting thresholds used later in this file.
...
#   - MRDetect_output_winter_2025/Processed_R_outputs/
#       BM_muts_plots_baseline/cfWGS_MRDetect_BM_data_updated_Feb2026.csv
...
#   - Spring 2026 X Plus 0% dilution metadata and feature files located by the
#     dilution helper functions in helpers.R.
#
# Outputs:
...
#     (Extended Data Figure 7D; historical filename).
#   - Output_tables_2025/Supplementary_Table_7_Dilution_Series_Combined_updated.xlsx
#     and CSV/source-data companions.
#
# Dependencies:
...
#   dilution-series samples. It describes dilution response independently from
#   the training cohort but does not refit or retune a classifier.
#
# Author:    Dory Abelman
...
# ── 1. FILE PATHS ───────────────────────────────────────────────────────────
# Keep paths project-relative so the script runs from a fresh checkout after
# the expected input folders are staged.
...
# Preserve the complete 48-library object for Figure 3C, Supplementary Table 7,
# and the ED5D/ED7D dilution panels. The temporary dilution_df object below is
# the older 45-library compatibility subset without the three new physical 0%
# controls; the complete object is restored where those manuscript outputs are
# constructed.
...
if (nrow(dilution_df) != 45L) {
  stop(
    "The 45-library compatibility subset expected 45 scored non-zero/new plus historical rows; observed ",
    nrow(dilution_df), ".",
    call. = FALSE
  )
}
...
if (nrow(dilution_df_clean) != 48L || ncol(dilution_df_clean) != 88L) {
  stop(
    "Supplementary Table 7 validation expected 48 libraries and 88 de-identified scored fields; observed ",
...
# The correlation sheet is calculated directly from the same 48 library rows
# exported in Supplementary Table 7. This keeps all four 0% references in the
...
if (!file.copy(
  ...
  unlink(supp_table7_temporary)
  stop("Failed to replace the final Supplementary Table 7 workbook.", call. = FALSE)
}
unlink(supp_table7_temporary)

# CSV copies of the two Supplementary Table 7 workbook sheets.
...
#
# Why it is here:
#   This validated workbook is used for Supplementary Table 7 and is copied to
#   each corresponding manuscript-output folder.
# -------------------------------------------------------------------------
...
# shared physical 0% diluent). The historically named
# dilution_df_release_only object retains the 45-library compatibility subset
# for the later fragmentomics-only display.
...
#
# What this is:
#   BM-informed dilution-response panel with healthy-control overlay and
...
ms_copy_artifact(
  ...
  artifact_id = "EDFIG5D",
  role = "figure_panel_png",
  description = "BM-informed dilution-response panel used as Extended Data Figure 5D.",
  script_name = "3_1_part2_Apply_cfWGS_thresholds_to_dilution_series.R"
)
...
#
# What this is:
#   Blood/cfDNA-informed dilution-response panel with healthy-control overlay
...
ms_copy_artifact(
  ...
  artifact_id = "EDFIG7D",
  role = "figure_panel_png",
  description = "Blood/cfDNA-informed dilution-response panel used as Extended Data Figure 7D.",
  script_name = "3_1_part2_Apply_cfWGS_thresholds_to_dilution_series.R"
)
...
# Build the complete 48-library long-form cfDNA source table. Patient and grouping
...
# -------------------------------------------------------------------------
# Complete 48-library version plus historical-series 0% endpoint sensitivity
#
...
# Build the complete 48-library cfDNA sensitivity source. Historical endpoints are
